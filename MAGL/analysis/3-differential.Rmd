...
```{r collapse = TRUE, fig.wide = TRUE, fig.height = 18}
ps <- lapply(kids, function(k)
    lapply(top_gs[[k]], function(g) {
      df$u <- logcounts(sce)[g, ]
      ggplot(df, aes(x = tSNE_1, y = tSNE_2, col = u)) +
...
plot_grid(plotlist = Reduce("c", ps), ncol = 4)
```
