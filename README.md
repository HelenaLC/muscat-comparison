...
Analyses discussed in the results section **Differential state analysis of mouse cortex exposed to LPS treatment** are provided as a browsable `workflowr`[^1] website [HERE](http://htmlpreview.github.io/?https://github.com/HelenaLC/muscat-comparison/blob/master/MAGL/docs/index.html).
...
script      | description 
------------|------------------------------------------------
`prep_X`    | generates a references SCE for simulation (e.g., by keeping samples from one condition only,<br>and unifying relevant cell metadata column names to `"cluster_id"`, `"sample_id"`, and `"group_id"`)
`prep_sim` | prepares a reference SCE for simulation by i) retaining subpopulation-sample combinations<br>with at least 100 cells; and, ii) estimating cell / gene parameters (offsets / coefficients and dispersions)
...
`plot_perf_cat`     | plots TPR-FDR-points across DD categories for a given p-value adjustment type (`p_adj.loc` or `p_adj.glb`)
