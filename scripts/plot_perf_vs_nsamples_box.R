...
gis <- .get_gis("data/sim_data/kang/de10_ns;simrep=1.rds")
df <- .tidy_res(list.files("results/kang/de10_ns", full.names = TRUE), gis) %>% 
    data.table %>% split(
...
df <- lapply(seq_along(res), function(i) {
    ...
    fdrtpr(perf) %>% select(c("TPR", "FDR", "method", "splitval")) %>% 
        rename(method = "n_samples", splitval = "method") %>% 
...
# prep. for plotting
gg_df <- df %>% 
    mutate_at("n_samples", as.numeric) %>% 
    mutate_at("method", function(u) 
        gsub("^method:", "", u) %>% 
            factor(levels = names(method_colors))) %>% 
    melt(id.vars = c("method", "n_samples", "rep"))

p <- ggplot(gg_df, aes(x = n_samples, y = value, 
    group = paste(method, n_samples), col = method, fill = method)) + 
    ...
    scale_x_continuous(breaks = unique(gg_df$n_samples)) +
