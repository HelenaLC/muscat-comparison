...
cols <- setNames(brewer.pal(nlevels(df$n_cells), "RdYlGn"), lvls)
...
p <- .plot_perf_curves(u)
p$facet$params$ncol <- 4
p$layers[[1]]$aes_params$size <- 0.2 # dashed lines at thrs
p$layers[[2]]$aes_params$size <- 0.3 # lines
p$layers[[3]]$aes_params$stroke <- 0.6 # points
p$guides$colour <- guide_legend("n_cells", override.aes = list(size = 1))
